...
class Action:
    ...
    def grounded(self, var_values):
        """
        var_values: dictionary of variables with their assigned values
        returns an action with the variables replaced by values
        """
        def grounded_signature(signature, var_values):
            gs = []
            for v in signature:
                if v[0].startswith('?') and var_values.has_key(v[0]):
                    gs.append((var_values[v[0]], v[1]))
                else:
                    gs.append(v)
            return gs
                
        signature = grounded_signature(self.signature, var_values)
        
        effect = Effect()
        for e in self.effect.addlist:
            effect.addlist.append(Predicate(e.name, grounded_signature(e.signature, var_values)))
        for e in self.effect.dellist:
            effect.dellist.append(Predicate(e.name, grounded_signature(e.signature, var_values)))
        precondition = [Predicate(p[0].name, grounded_signature(p[0].signature, var_values)) for p in self.precondition]
            
        return Action(self.name, signature, precondition, effect)
