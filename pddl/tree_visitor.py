...
class TraversePDDLDomain(PDDLVisitor):
    ...
    def visit_variable(self, node):
        ...
        if not node.typed:
            ...
            if len(node.types)>1:
                raise SemanticError('Error, unsupported multiple types for variable ' + node.name)
            #typelist = list()
            #for t in node.types:
            ...
            self.set_in(node, (node.name, node.types[0]))
    ...
    def visit_precondition_stmt(self, node):
        ...
        if formula.key == 'and':
            ...
            for c in formula.children:
                ...
                isNegative = False
                if c.key == 'not':
                    # This is a negative precondition, only one child allowed.
                    if len(c.children) != 1:
                        raise SemanticError('Error not statement with multiple '
                                            'children in predicate of action')
                ...
                # Call helper.
                self.add_precond(precond, nextPredicate, isNegative)
        else:
